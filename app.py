...
# 3. Código HTML, CSS y JS con Gestos Táctiles (Swipe)
html_mobile = """
<!DOCTYPE html>
<html lang="es">
<head>
...
<style>
    ...
    body, html {
        ...
        /* Evita el scroll nativo en móviles para que el swipe funcione perfecto */
        touch-action: pan-y;
        overflow: hidden;
    }
    ...
    /* Zonas táctiles ajustadas: 25% izquierda (atrás), 75% derecha (adelante) */
    .touch-area { position: absolute; top: 0; height: 100%; z-index: 10; cursor: pointer; }
    .touch-left { left: 0; width: 25%; }
    .touch-right { right: 0; width: 75%; }
...
<div id="mobile-deck">
    ...
    <div class="slide active" id="m-slide-1">
        ...
        <div style="margin-top: auto; font-size: 13px; color: #888; text-align: center; font-weight: 600;">Desliza o toca la derecha 👉</div>
    </div>
...
<script>
    ...
    // --- LÓGICA PARA GESTOS SWIPE (Deslizar) ---
    let touchstartX = 0;
    let touchendX = 0;

    function handleSwipe() {
        // Si el usuario desliza hacia la izquierda (avanzar)
        if (touchendX < touchstartX - 40) move(1);
        // Si el usuario desliza hacia la derecha (retroceder)
        if (touchendX > touchstartX + 40) move(-1);
    }
...
components.html(html_mobile, height=750, scrolling=False)import streamlit as st
import streamlit.components.v1 as components
